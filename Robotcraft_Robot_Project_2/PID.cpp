...
PID::PID(float p, float i, float d, uint8_t dirPin, Adafruit_MCP4728 *DAC, uint8_t channel //, uint8_t stepPin
         ) :
    Motor(dirPin), channel(channel) //, stepPin)
{
  this->DAC = DAC;
  this->p = p;
  this->i = i;
...
float PID::calc(float desired, float real)
{
  ...
  float result = this->p * this->Proportional.getValue() + this->i * this->Integral.getValue() * DELTA_T
...
void PID::apply(int16_t PIDresult)
{
  ...
  //TODO:analog data set
  this->DAC->setChannelValue(static_cast<MCP4728_channel_t>(this->channel), static_cast<uint16_t>(abs(PIDresult)));

...
Robot::Robot(uint8_t leftSensorPin, uint8_t rightSensorPin, uint8_t frontSensorPin, uint8_t leftEncoderPin1,
             uint8_t leftEncoderPin2, float leftP, float leftI, float leftD, uint8_t leftDirPin,
             Adafruit_MCP4728 *leftDAC, uint8_t leftChannel,
             //uint8_t leftStepPin,
    ...
    leftSensor(leftSensorPin), rightSensor(rightSensorPin), frontSensor(frontSensorPin), leftWheel(leftEncoderPin1,
                                                                                                   leftEncoderPin2,
                                                                                                   leftP, leftI, leftD,
                                                                                                   leftDirPin, leftDAC,
                                                                                                   leftChannel
        ...
        rightEncoderPin1, rightEncoderPin2, rightP, rightI, rightD, rightDirPin, rightDAC, rightChannel //, rightStepPin
        ), pixels(neoPixelNum, neoPixelPin,
    NEO_GRB + NEO_KHZ800)
{
...
void Robot::updateWheelsVelocities()
{
  ...
  TWO_PI * ROBOT_R * (rightWheel.encoder.getChange() + leftWheel.encoder.getChange()) / ROBOT_C / 2.0 / DELTA_T;
  this->realVelocity.angular.z =
  TWO_PI * ROBOT_R * (rightWheel.encoder.getChange() - leftWheel.encoder.getChange()) / ROBOT_C / ROBOT_B / DELTA_T;

  this->leftWheel.setRealVelocity(
      (this->realVelocity.linear.x - this->ROBOT_B / 2 * this->realVelocity.angular.z) / this->ROBOT_R);
  this->rightWheel.setRealVelocity(
      (this->realVelocity.linear.x + this->ROBOT_B / 2 * this->realVelocity.angular.z) / this->ROBOT_R);

  this->leftWheel.setDesiredVelocity(
      (this->desiredVelocity.linear.x - this->ROBOT_B / 2 * this->desiredVelocity.angular.z) / this->ROBOT_R);
...
{
  this->position.theta = atan2(sin(this->position.theta + this->realVelocity.angular.z * DELTA_T),
                               cos(this->position.theta + this->realVelocity.angular.z * DELTA_T));
  this->position.x = this->position.x + realVelocity.linear.x * DELTA_T * cos(this->position.theta);
  this->position.y = this->position.y + realVelocity.linear.x * DELTA_T * sin(this->position.theta);
